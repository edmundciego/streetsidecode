...
@section('title', translate('sms_config'))
        ...
        <div class="content container-fluid">
            ...
            <div class="row">
                @foreach($data_values as $gateway)
                    @php
                        $live_values_raw = $data_values->where('key_name', $gateway->key_name)->first()->live_values;
                        $live_values = is_string($live_values_raw) ? json_decode($live_values_raw, true) : $live_values_raw;
                    @endphp
            ...
            <div class="card mt-4">
                <div class="card-header">
                    <h4 class="page-title">{{ translate('Send Test SMS') }}</h4>
                </div>
                <div class="card-body p-30">
                    <form action="{{ route('configuration.send-test-sms') }}" method="POST">
                        @csrf
                        ...
                        <div class="d-flex justify-content-end">
                            <button type="submit" class="btn btn--primary">{{ translate('Send Test SMS') }}</button>
                        </div>
                    </form>
                </div>
            </div>
